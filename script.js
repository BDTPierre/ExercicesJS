...
function addItem() {
    let item = randomFruits[Math.floor(Math.random() * randomFruits.length)];
    // document.getElementById('additem').innerText = item
    let ul = document.getElementById('displayList')
    let li = document.createElement('li')
    li.innerHTML = item;
    ul.appendChild(li)
...
for (let i = 0; i < persons.length; i++) {
    let li = document.createElement('li')
    li.innerHTML = persons[i].id + " " + persons[i].name + " " + (persons[i].student ? 'Etudiant')
    ul.appendChild(li);
    }
}
displayObjectList()
